...
def infer_1_image(path, model, transform, classnames = [], device = "cpu"):
...
def test(config):
    ...
    if os.path.isdir(config['test']['path']):  
        ...
        for imagelink in tqdm(os.listdir(config['test']['path'])):
            ...
            image = infer_1_image(path = os.path.join(config['test']['path'], imagelink), 
                                  model = model,
                                  transform = transform,
                                  classnames = config['class']['name'],
                                  device = device)
        ...
        image = infer_1_image(path = config['test']['path'], 
                              model = model,
                              transform = transform,
                              classnames = config['class']['name'],
                              device = device)
